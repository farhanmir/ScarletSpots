...
/// `ios/scripts/build_sqlite.py` from the canonical JSON sources. No
...
extension Database {
    ...
    static let shared: Database = {
        guard let db = Database.openBundled(resource: "scarletspots") else {
            fatalError("scarletspots.sqlite is missing from the app bundle. Regenerate it with ios/scripts/build_sqlite.py")
        }
        return db
    }()
}
