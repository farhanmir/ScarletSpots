...
// Shared design tokens + components for the native auth surfaces. Values are
// intentionally stable (hex codes, radii, heights, paddings) so auth flows stay
// visually consistent release to release.
...
enum NativeAuthColors {
    ...
    // Tailwind-like zinc scale used throughout auth components.
    ...
    // Occupancy palette used across native map/search surfaces.
