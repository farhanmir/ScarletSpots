...
final class AutoParkCoordinator: ObservableObject {
    ...
    private func startSensing() {
        ...
        isRunning = true
        Logger.event("autopark", "sensing_started")
    ...
    private func stopSensing() {
        isRunning = false
        Logger.event("autopark", "sensing_stopped")
    ...
    private func handleLocationUpdate(_ location: CLLocation) async {
        Logger.event(
            ...
            metadata: [
                ...
                "lng": String(format: "%.6f", location.coordinate.longitude),
                "accuracyM": String(format: "%.1f", location.horizontalAccuracy),
        ...
        guard Date().timeIntervalSince(lastDecisionAt ?? .distantPast) >= locationThrottle else {
            Logger.event("autopark.location", "location_throttled", metadata: ["seconds": String(format: "%.1f", Date().timeIntervalSince(lastDecisionAt ?? .distantPast))])
    ...
    func handleTrigger(
        ...
        guard !isInFlight else {
            ...
            publishSnapshot(
                ...
                explanation: "A start/end request is already in flight, so this trigger was suppressed to avoid duplicate mutations.",
                confidence: nil,
                lot: resolvedLot(for: preferredLocation ?? LocationEngine.shared.latestLocation),
                location: preferredLocation ?? LocationEngine.shared.latestLocation,
                checks: [],
                appendToHistory: true
            )
            return
        ...
        Logger.event(
            ...
            metadata: [
                ...
                "wakeReason": wakeReason,
                "kind": kind,
                "hasLocation": location == nil ? "false" : "true"
            ]
        )
        if kind == "end" {
            let evaluation = evaluateEnd(source: source, location: location)
            logEvaluationSummary(kind: "end", source: source, evaluationReason: evaluation.reason, hardPass: evaluation.hardPass, confidence: evaluation.confidence, checks: evaluation.checks)
        ...
        logEvaluationSummary(kind: "start", source: source, evaluationReason: evaluation.reason, hardPass: evaluation.hardPass, confidence: evaluation.confidence, checks: evaluation.checks)
        publishSnapshot(
            phase: "decision",
            wakeReason: wakeReason,
            monitoringMode: currentMonitoringMode(),
    ...
    private func commitStart(candidate: ParkingCandidate, wakeReason: String) async {
        ...
        do {
            ...
            Logger.event(
                "autopark.mutation",
                "start_success",
                metadata: ["lotId": candidate.lotId, "source": candidate.source]
            )
            publishSnapshot(
                phase: "mutation",
                wakeReason: wakeReason,
    ...
    private func commitEnd(source: String, location: CLLocation?, confidence: Double?) async {
        ...
        do {
            ...
            await NativeSessionStore.shared.refresh()
            clearParkedState()
            Logger.event("autopark.mutation", "end_success", metadata: ["source": source])
            publishSnapshot(
                phase: "mutation",
                wakeReason: currentWakeReason,
                monitoringMode: currentMonitoringMode(),
            ...
            Logger.event(
                "autopark.mutation",
                "end_queued_offline",
                metadata: ["source": source, "error": error.localizedDescription],
                level: "warn"
            )
            publishSnapshot(
                phase: "mutation",
    ...
    private func evaluateEnd(source: String, location: CLLocation?) -> EndEvaluation {
        ...
        if source == "audio_reconnect" {
            let disconnectAge = AudioRouteEngine.shared.lastDisconnectAt.map { Date().timeIntervalSince($0) }
            let reconnectFresh = disconnectAge != nil && (disconnectAge ?? .infinity) <= audioReconnectMaxDisconnectAge
            add(
                "audio_reconnect_context",
                "Recent disconnect context",
                reconnectFresh,
                reconnectFresh
                    ? "\(Int((disconnectAge ?? 0).rounded()))s since disconnect"
                    : "No recent disconnect context",
                "audio_reconnect_without_recent_disconnect"
            )
            guard reconnectFresh else {
                return EndEvaluation(
                    checks: checks,
                    confidence: nil,
                    reason: "audio_reconnect_without_recent_disconnect",
                    explanation: "Audio reconnect was observed without a recent disconnect context, so Auto-End blocked to avoid a false end.",
                    hardPass: false
                )
            }
    ...
    private func logEvaluationSummary(
        ...
        let failedKeys = failed.map(\.key).joined(separator: ",")
        let failedCodes = failed.compactMap(\.reasonCode).joined(separator: ",")
        Logger.event(
            "autopark.gates",
            hardPass ? "evaluation_passed" : "evaluation_blocked",
            metadata: [
                "kind": kind,
                "source": source,
                "reason": evaluationReason,
                "confidence": confidence.map { String(format: "%.2f", $0) } ?? "n/a",
                "failedGateKeys": failedKeys.isEmpty ? "none" : failedKeys,
                "failedReasonCodes": failedCodes.isEmpty ? "none" : failedCodes
            ],
            level: hardPass ? "info" : "warn"
        )
    }
